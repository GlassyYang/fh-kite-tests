...
{
  Options options;
  options.sendInterest = false;
  options.sendData = false;
  options.sendNack = false;
  std::string prefix;

  po::options_description visibleDesc("Options");
  visibleDesc.add_options()
    ("help,h",      "print this help message and exit")
    ("mp-listening,l", po::bool_switch(&options.mpListen), "listening interest from mp. just for test")
    ("mp-name,m", po::value<std::string>(), "mobile producer prefix, which will be listen")
    ("send-nack,n", po::bool_switch(&options.sendNack), "when receive interest to mp, send nack. just for test.")
    ("send-data,d", po::bool_switch(&options.sendData), "send data when receive mp interest. just for test")
    ("send-interest,i", po::bool_switch(&options.sendInterest), "send interest to mp. just for test.")
  ...
  if (vm.count("mp-name") > 0) {
    options.mpName = vm["mp-name"].as<std::string>();
  }

  if (options.mpListen && vm.count("mp-name") == 0)
  {
    std::cerr << "when enable listening mp prefix, should give a mp prefix" << std::endl;
    return 1;
  }

  if(options.sendData && options.sendNack) {
    std::cerr << "cannot send both nack and data" << std::endl;
    return 1;
  }

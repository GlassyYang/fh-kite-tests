...
BOOST_AUTO_TEST_CASE(DecodeBad)
{
  ...
  Name kiteAckName = makeRequest().makeInterest(signer, si).getName();

  // invalid content type
  data.setName(kiteAckName);
  data.setContentType(tlv::ContentType_Key);
  BOOST_CHECK_THROW(Ack kiteAck(data), Ack::Error);
...
BOOST_AUTO_TEST_CASE(EndToEnd)
{
  ...
  security::SigningInfo si;
  si.setSignedInterestFormat(security::SignedInterestFormat::V03);
  producerFace.expressInterest(req.makeInterest(signer, si), nullptr, nullptr, nullptr);

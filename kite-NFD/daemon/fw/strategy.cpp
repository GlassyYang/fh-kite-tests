...
Strategy::lookupFib(const pit::Entry& pitEntry) const
{
  ...
  for (const Delegation& del : fh) {
    if(del.preference == tlv::ContentType_KiteAck) {
      continue;
    }
  ...
  if (fibEntry == nullptr) {
    fibEntry = &fib.findLongestPrefixMatch(pitEntry);
  }
...
const fib::Entry&
Strategy::lookupFib(const Name& name) const {
  const Fib& fib = m_forwarder.getFib();
  return fib.findLongestPrefixMatch(name);
}
} // namespace fw
} // namespace nfd

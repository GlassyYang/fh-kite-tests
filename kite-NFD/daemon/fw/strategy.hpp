...
protected: // accessors
  /** \brief Performs a FIB lookup, considering Link object if present except KITE forwarding hint.
   * \param pitEntry the PIT entry
   */
  const fib::Entry&
  lookupFib(const pit::Entry& pitEntry) const;
  /** \brief Performs a FIB lookup by name.
   * \param name the name
   */
  const fib::Entry&
  lookupFib(const Name& name) const;

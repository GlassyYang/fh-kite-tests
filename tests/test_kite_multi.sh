...
NFD3_TEST="NDN_LOG=kite.*=ALL kiteproducer -r /kite-test/rv -p /kite-test/alice -l 1000000 >nfd3.log 2>&1"
...
if [[ "$res3" =~ "Received consumer Interest" && "$res4" =~ "Received consumer Interest" ]]; then
    ...
    echo "test failed! kite producer doesn't receive packet: $res3 $res4 " >&2
fi